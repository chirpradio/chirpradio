...
class ChirpApi {
         ...
         if ($request['q'] == 'playlist/current') {
             ...
             if ($method == 'POST') {
               ...
               $response = $this->create(
                 ...
                 $request['dj_name'],
                 $request['time_played'],
                 $request['playlist_track_id']);
             }
             else {
               throw new InvalidRequestException($request['q'] . " requires a HTTP POST.");
             }
         }
         else if (strpos($request['q'], 'playlist/delete/website/') === 0) {
           $id = substr($request['q'], strrpos($request['q'], '/') + 1);

           if ($method == 'DELETE') {
               $response = $this->delete($id, 'website');
             }
           else {
             throw new InvalidRequestException($request['q'] . " requires a HTTP DELETE.");
           }
         }
         else if (strpos($request['q'], 'playlist/delete/playlist-app/') === 0) {
           $id = substr($request['q'], strrpos($request['q'], '/') + 1);

           if ($method == 'DELETE') {
               $response = $this->delete($id, 'playlist-app');
             }
           else {
             throw new InvalidRequestException($request['q'] . " requires a HTTP DELETE.");
           }
  ...
  private function create($track_name, $track_album, $track_artist, $track_label, 
                          $dj_name, $time_played, $playlist_track_id) {
    ...
    if ($result = mysql_query($query)) {
      ...
      $query = sprintf("UPDATE textpattern SET url_title = '%s' WHERE ID = %d",
                       ...
                       $track_id);
      if ($result = mysql_query($query)) {
        $response = json_encode(array('track_id' => $track_id)) . "\n";
      }
      else {
  ...
  private function delete($id, $id_type) {
    if ($id_type == 'website') {
      $select_query = sprintf("SELECT ID AS track_id, custom_3 AS playlist_track_id FROM textpattern WHERE ID = %d", $id);
      $delete_query = sprintf("DELETE FROM textpattern WHERE ID = %d", 
                       $id);
    }
    else if ($id_type = 'playlist-app') {
      $select_query = sprintf("SELECT ID AS track_id, custom_3 AS playlist_track_id FROM textpattern WHERE custom_3 = '%d'", $id);
    ...
    if ($result = mysql_query($select_query)) {
      ...
      throw new NotFoundException('Track with ID ' . $id . ' and type ' . $id_type . ' not found.'); 
    }
